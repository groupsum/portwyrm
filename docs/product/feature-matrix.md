...
| UI-01 | P | Surfaces | Setup, login/2FA, dashboard counts, proxy/redirection/stream/dead tables, access lists, certificates, users, audit log, settings, profile/password/2FA. | Responsive UI exposes every permitted operation, hides disallowed sections/actions, and presents audit records as Time, Actor, Resource, Action, Outcome, and Details. | [S14] |

...
export default function AccessTokensView({currentUser, onList, onCreate, onRotate, onRevoke}: Props) {
  ...
  return <div className="space-y-6 animate-in fade-in duration-200">
    <section aria-labelledby="access-tokens-title" className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-3xs dark:border-zinc-800 dark:bg-zinc-900">
      <header className="flex items-start justify-between gap-4 border-b border-slate-100 bg-slate-50 px-6 py-5 dark:border-zinc-800 dark:bg-zinc-900/50">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {creating ? <form onSubmit={create} className="mb-6 space-y-5 rounded-2xl border border-indigo-200 bg-indigo-50/40 p-5 dark:border-indigo-950 dark:bg-indigo-950/10">
          ...
          <div className="grid gap-4 sm:grid-cols-2"><label className="text-xs font-bold">Token name<input autoFocus value={name} onChange={event => setName(event.target.value)} maxLength={100} placeholder="e.g. Deployment automation" required className="mt-1.5 w-full rounded-xl border border-slate-200 bg-white p-2.5 dark:border-zinc-700 dark:bg-zinc-950" /></label><label className="text-xs font-bold">Expires<select value={expiry} onChange={event => setExpiry(event.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-200 bg-white p-2.5 dark:border-zinc-700 dark:bg-zinc-950"><option value="30">30 days</option><option value="90">90 days</option><option value="365">1 year</option><option value="never">Never</option></select></label></div>
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 dark:border-zinc-800">
          {loading ? <div className="grid min-h-40 place-items-center"><Loader2 className="h-6 w-6 animate-spin text-indigo-600" /></div> : <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-xs"><thead className="border-b border-slate-200 bg-slate-50 text-[10px] uppercase tracking-wider text-slate-500 dark:border-zinc-800 dark:bg-zinc-900"><tr><th className="px-4 py-3">Name</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Created</th><th className="px-4 py-3">Last used</th><th className="px-4 py-3">Expires</th><th className="px-4 py-3 text-right">Manage</th></tr></thead><tbody className="divide-y divide-slate-100 dark:divide-zinc-800">{filtered.length === 0 ? <tr><td colSpan={6} className="px-6 py-12 text-center"><KeyRound className="mx-auto h-8 w-8 text-slate-300" /><h3 className="mt-3 text-sm font-extrabold">{search ? 'No matching tokens' : 'No access tokens yet'}</h3><p className="mt-1 text-xs text-slate-500">{search ? 'Try another search.' : 'Create a scoped token for an integration or API workflow.'}</p></td></tr> : filtered.map(token => { const status = tokenStatus(token); return <tr key={token.id} className="hover:bg-slate-50/70 dark:hover:bg-zinc-800/30"><td className="px-4 py-3"><div className="max-w-64 truncate font-extrabold text-slate-900 dark:text-zinc-100">{token.name}</div><div className="mt-1 max-w-64 truncate font-mono text-[10px] text-slate-400">{token.scopes.length === 1 && token.scopes[0] === 'user' ? 'Full account access' : `${token.scopes.filter(scope => scope !== 'user').length} scoped permissions`}</div></td><td className="px-4 py-3"><span className={`rounded-md px-1.5 py-0.5 text-[9px] font-extrabold uppercase ${status === 'Active' ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-400' : 'bg-slate-100 text-slate-500 dark:bg-zinc-800'}`}>{status}</span></td><td className="px-4 py-3 text-slate-500">{timeLabel(token.createdAt)}</td><td className="px-4 py-3 text-slate-500">{timeLabel(token.lastUsedAt)}</td><td className="px-4 py-3 text-slate-500"><Clock3 className="mr-1 inline h-3 w-3" />{timeLabel(token.expiresAt)}</td><td className="px-4 py-3 text-right"><button type="button" onClick={() => setActionTokenId(token.id)} disabled={status !== 'Active' || busy} aria-label={`Actions for ${token.name}`} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 disabled:opacity-30 dark:hover:bg-zinc-800"><MoreVertical className="h-4 w-4" /></button></td></tr>; })}</tbody></table></div>}
        </div>
      </div>
    </section>

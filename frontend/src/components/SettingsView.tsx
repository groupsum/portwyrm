  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      {activeTab === 'mfa' && <section id="settings-panel-mfa" role="tabpanel" aria-labelledby="settings-tab-mfa" className="space-y-5">
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_20rem]">
          <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-3xs dark:border-zinc-800 dark:bg-zinc-900">
            ...
            <div className="divide-y divide-slate-100 dark:divide-zinc-800">{users.map(user => <div key={user.id} className="flex items-center justify-between gap-4 px-5 py-3"><span className="min-w-0"><strong className="block truncate text-xs">{user.displayName}</strong><small className="text-[10px] text-slate-600 dark:text-zinc-400">{user.username} · {user.role}</small></span><span className={`shrink-0 rounded-full px-2.5 py-1 text-[10px] font-extrabold ${user.mfa ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-400' : 'bg-slate-100 text-slate-700 dark:bg-zinc-800 dark:text-zinc-300'}`}>{user.mfa ? 'Enrolled' : 'Not enrolled'}</span></div>)}</div>
          </div>
      ...
      {activeTab === 'runtime' && <section id="settings-panel-runtime" role="tabpanel" aria-labelledby="settings-tab-runtime" className="grid gap-5 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-3xs dark:border-zinc-800 dark:bg-zinc-900"><h3 className="mb-4 flex items-center gap-2 text-sm font-extrabold"><Cpu className="h-5 w-5 text-indigo-500" />Proxy runtime</h3><dl className="space-y-3 text-xs"><div className="flex justify-between rounded-xl bg-slate-50 p-3 dark:bg-zinc-950"><dt className="text-slate-500">Nginx engine</dt><dd className="font-bold">{health.nginxState}</dd></div><div className="flex justify-between gap-3 rounded-xl bg-slate-50 p-3 dark:bg-zinc-950"><dt className="text-slate-500">Applied generation</dt><dd title={health.currentGeneration || 'No active generation'} className="max-w-48 truncate font-mono font-bold">{health.currentGeneration || 'Unavailable'}</dd></div><div className="flex justify-between rounded-xl bg-slate-50 p-3 dark:bg-zinc-950"><dt className="text-slate-500">Certificate scheduler</dt><dd className="font-bold text-emerald-600">{health.schedulerState}</dd></div></dl></div>

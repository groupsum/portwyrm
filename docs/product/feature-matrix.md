...
|---|---|---|---|---|---|
| ID-01 | P | First run | Setup wizard creates the first active user; unauthenticated user creation is allowed only while no active user exists, and the first user is forced admin. Optional `INITIAL_ADMIN_EMAIL`/`INITIAL_ADMIN_PASSWORD` skips the wizard. | The API preserves both bootstrap paths. The OCI image safely selects the environment-bootstrap path with `admin@example.com` and a generated one-time password that is revealed only in the first-start log. The superuser must replace it before accessing the control plane, and the plaintext bootstrap file is removed after the change commits. Post-bootstrap unauthenticated creation remains denied. | [S4][S6] |
...
| PH-01 | P | Proxy host CRUD | Create/list/get/update/soft-delete and explicit enable/disable. Each row has owner, timestamps, domains, enabled flag, and Nginx online/error metadata. The UI distinguishes human, automation-managed, and system provenance and displays any automation manager verbatim. | API, UI, ownership/provenance, audit, toggle idempotency/error semantics, and generated state match. | [S2][S10] |
...
| CE-05 | P | Lifecycle | List/get with expiry and ownership; manual renew (Let’s Encrypt only); automatic renewal when within 30 days; download Let’s Encrypt material as zip; delete/soft-delete; host relations across proxy/redirection/dead/stream. | Certificate mutations persist the authenticated owner and automation provenance; absent ownership is displayed as unassigned rather than system. Timer and manual renewal tests, relation/in-use behavior, audit, regenerated referencing configs. | [S3][S12]; `tests/runtime/test_certificates_runtime.py`, `tests/runtime/test_migration_runtime.py`, `tests/runtime/test_ui_assets.py` |
...
| NG-02 | P | Default site | Unknown Host behavior: congratulations page, 404, Nginx 444, redirect, or custom HTML. Admin-only setting. | Secure default is Nginx 444 so unmatched hosts disclose no service; all five modes remain renderer-supported, and the opt-in congratulations response is explicit text/plain. Evidence: [secure unmatched-host default](../../artifacts/secure-unmatched-host-default-2026-07-25.md). | [S8][S10] |
...
| AU-01 | P | Audit | Admin-only audit event list/get; events store initiating actor, executing subsystem, object type/id, action, timestamp, metadata; expandable user; searchable UI/API surface. | Create/update/delete/toggle/password/permission/cert actions emit redacted, immutable events; reconciliation preserves the authenticated initiator while identifying the Portwyrm reconciler as executor. | [S3][S6][S10]; `tests/runtime/test_tigrbl_app_runtime.py` |
| UI-01 | P | Surfaces | Setup, login/2FA, dashboard counts, proxy/redirection/stream/dead tables, access lists, certificates, users, audit log, settings, profile/password/2FA. | Responsive UI exposes every permitted operation, hides disallowed sections/actions, and presents audit records as Time, Initiated By, Executed By, Resource, Action, Outcome, and Details. Resource provenance distinguishes human, automation, system, and unassigned ownership without duplicating an owner name as its manager. | [S14]; `tests/runtime/test_ui_assets.py` |
